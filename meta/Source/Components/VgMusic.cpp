	...
	m_CtrlPanel_CheckBox.SetCheckedChangedCallback(
		[this](HyCheckBox *pThis)
		{
			if(pThis->IsChecked() && m_MusicTrackList.empty())
				m_MusicTrackList = Compositorium::Get()->GetMusicPlayList(CONSOLE_NES | CONSOLE_SNES | CONSOLE_Genesis | CONSOLE_N64 | CONSOLE_Dreamcast | CONSOLE_PS1 | CONSOLE_PS2 | CONSOLE_PC);
		});
